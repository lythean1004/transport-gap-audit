...
def prep_crosscheck(m_df, pub_df, routes_to_exclude=None):
    ...
    # 보정 규칙 미정(결정 7번)에 따른 구조적 NULL 플레이스홀더
...
def main():
    ...
    # [I3] 평일 검증
    if not ledger.empty:
        obs_dates_ts = pd.to_datetime(ledger['obs_date'].unique())
    ...
    for zero_flag, name, m_full, ev_src in [(False, 'Base(결측유지)', m_base_all, ev_base), (True, 'Zero-Fill(0처리)', m_zero_all, ev_zero)]:
        for d_list, w_name in [(dates_core, '본안(3일)'), (dates_supp, '부속(4일)')]:
            ...
            if not sub_m.empty:
                ...
                n_insuff = int(sub_m['insufficient_observation'].sum())
                n_groups = len(sub_m)
                insuff_rate = float((n_insuff / n_groups) * 100) if n_groups > 0 else 0.0
                n_events = len(sub_ev[~sub_ev['routeid'].isin(exc_routes)])
            else:
                med_val, p25_val, p75_val, insuff_rate, n_groups, n_events, n_insuff = pd.NA, pd.NA, pd.NA, pd.NA, 0, 0, 0
            ...
            sens_res.append({
                ...
                'Groups': n_groups,
                'Events': n_events,
                'Insuff(%)': f"{insuff_rate:.1f}% ({n_insuff}/{n_groups})" if pd.notna(insuff_rate) else "N/A",
                '_raw_med': med_val,
                '_raw_p25': p25_val,
                '_raw_p75': p75_val,
...
- **실패 모드 (b) 원장 행 부재 (`ledger row missing`)**:
  ...
  - 발생 건수: 전체 관측 기간 총 {total_missing_all}건
    ...
    * 정규 관측일 (2026-09-07~2026-09-11): 총 {missing_regular_tot}건 (그 중 본안 3일 기간 총 {total_missing_core}건)."""
    ...
    if args.dry_run:
        # ==================================================
        # === [I1] insufficient 비율 2계열 진단 및 분모 명시 ===
        # ==================================================
        print("==================================================")
        print("=== [I1] insufficient 비율 2계열 진단 및 분모 명시 ===")
        print("==================================================")
        print("1. 실측 기반 insufficient 비율 (코드상 단일 정의 사용):")
        print("   * 산출식: (n_events < 4인 그룹 수) / (전체 그룹 수) × 100")
        print("   * 부속 Base 분모 95 원인: 2026-09-10 PM 창에서 GGB234000003 노선의 검출 이벤트가 0건이 되어 groupby 에서 누락됨 (Zero-Fill에서는 0건이 채워져 96그룹).")
        print("\n| 조건 | 분자(insuff 그룹) | 분모(전체 그룹) | 비율 |")
        print("|---|---|---|---|")
        b_c = sens_df.iloc[0]
        z_c = sens_df.iloc[2]
        b_s = sens_df.iloc[1]
        z_s = sens_df.iloc[3]
        print(f"| 본안 Base | {b_c['_n_insuff']} | {b_c['_n_groups']} | {b_c['_raw_insuff']:.1f}% |")
        print(f"| 본안 Zero | {z_c['_n_insuff']} | {z_c['_n_groups']} | {z_c['_raw_insuff']:.1f}% |")
        print(f"| 부속 Base | {b_s['_n_insuff']} | {b_s['_n_groups']} | {b_s['_raw_insuff']:.1f}% |")
        print(f"| 부속 Zero | {z_s['_n_insuff']} | {z_s['_n_groups']} | {z_s['_raw_insuff']:.1f}% |")
        print("\n2. 유저 제시값(5.6%, 4.2%, 12.5%, 6.9%) 대조 결과:")
        print("   * 위 표의 어떤 분모(72, 95, 96)에서도 정수 분자로 해당 퍼센티지를 정확히 산출할 수 없음.")
        print("   * 결론: 해당 수치들은 현행 v6 코드에 존재하지 않는 과거 버전(v2 등)의 팬텀 값임 (보고 후 수정하지 않음).")
        print("\n==================================================")
        print("=== [H1 최상단 배치] p25/p75 집계 정의 확정 및 v5 vs v6 대조 ===")
        print("==================================================")
        print("==================================================")
...
| agree_within_5min 건수 | {(out_completed['headway_source_agreement'] == 'agree_within_5min').sum()} | {(out_inclusive['headway_source_agreement'] == 'agree_within_5min').sum()} | {(out_core_crosscheck['headway_source_agreement'] == 'agree_within_5min').sum()} | {(out_supp['headway_source_agreement'] == 'agree_within_5min').sum()} |"""
        ...
        core_n_in = out_core_crosscheck['insufficient_observation'].sum()
        core_n_tot = len(out_core_crosscheck)
        supp_n_in = out_supp['insufficient_observation'].sum()
        supp_n_tot = len(out_supp)
        print(f"6. [지표 수용] insufficient_observation 비율(본안 {core_n_in/core_n_tot*100:.1f}% ({core_n_in}/{core_n_tot}), 부속 {supp_n_in/supp_n_tot*100:.1f}% ({supp_n_in}/{supp_n_tot})) 수용 및 노선×일자×창 단위 유지 여부")
        print("7. [첫차/막차] first_bus_ok / last_bus_ok 판정용 '기점~정류소 간 주행시간 보정 규칙' 정의 및 도입 여부")
        print("8. [공표 단위] 공표 배차 intervaltime 단위 '분' 확정 (공식 가이드라인 문서 근거 확보 완료)")
